...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Fonero users should be well used to small numbers with many decimal places by now :)
 ...
 *  The strategy used here is to fonero a small number of peers
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    // Fonero connections with certain characteristics

    // Deterministically select 4 peers to fonero by netgroup.
    ...
    // Fonero the 8 nodes with the best ping times.
    ...
    // Fonero 4 nodes that most recently sent us transactions.
    ...
    // Fonero 4 nodes that most recently sent us blocks.
    // An attacker cannot manipulate this metric without performing useful work.
    ...
    // Fonero the half of the remaining nodes which have been connected the longest.
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Fonero Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Fonero Core is probably already running."), addrBind.ToString());
        else
