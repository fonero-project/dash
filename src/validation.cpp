...
#if defined(NDEBUG)
# error "Fonero Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    {
    LOCK(pool.cs); // fonero pool.mapNextTx
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	   ...
	   if(nPrevHeight <= 54900) {
	     ...
	     nSubsidy = (20.000007 - (0.000009 * nPrevHeight)) * COIN;
	   }   
       } else {
...
//    assert(hashPrevBlock == view.GetBestBlock());
    ...
    // FNO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
...
//    }
    ...
    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        ...
        return state.DoS(0, error("ConnectBlock(FNO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // FNO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(FNO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(FNO): spork is off, skipping transaction locking checks\n");
    }
