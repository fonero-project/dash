...
const char * const BITCOIN_CONF_FILENAME = "azart.conf";
const char * const BITCOIN_PID_FILENAME = "azartd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "azart" is a composite category enabling all Azart-related debug output
            if(ptrCategory->count(string("azart"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override azart.conf
