...
// ***TODO*** ScanHash is not yet used in Fonero
//
...
{
    LogPrintf("FoneroMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fonero-miner");
    ...
    try {
        ...
        while (true) {
		    ...
		    LogPrintf("FoneroMiner -- loop: fvNodesEmpty: %d, IsInitialBlockDownload: %d, IsSynced:%d \n",!fvNodesEmpty, !IsInitialBlockDownload(), masternodeSync.IsSynced());
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("FoneroMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("FoneroMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FoneroMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("FoneroMiner -- runtime error: %s\n", e.what());
        return;
    }
}
