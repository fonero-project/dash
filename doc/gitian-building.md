...
*Setup instructions for a Gitian build of Fonero Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fonero
...
- [Building Fonero Core](#building-fonero-core)
...
Clone the git repositories for Fonero Core and Gitian.
...
in the Fonero Core repository under 'Fetch and build inputs' to install sources which require
...
To build Fonero Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Fonero Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fonero/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/fonero-project/fonero
...
For example:
```bash
URL=https://github.com/crowning-/fonero.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit fonero=${COMMIT} --url fonero=${URL} ../fonero/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fonero=${COMMIT} --url fonero=${URL} ../fonero/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fonero=${COMMIT} --url fonero=${URL} ../fonero/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fonero git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fonero/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fonero.git
SIGPATH=/some/root/path/fonero-detached-sigs.git

./bin/gbuild --url fonero=${BTCPATH},signature=${SIGPATH} ../fonero/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fonero-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fonero-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fonero-osx-build.assert
```
...
[fonero/gitian.sigs](https://github.com/fonero-project/gitian.sigs/) repository, or if that's not possible to create a pull
request.
